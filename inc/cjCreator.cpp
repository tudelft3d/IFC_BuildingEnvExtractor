...
template <typename T>
std::vector<std::vector<T>> subListScore(const std::vector<T>& inputList, const std::vector<int>& scoreList, int splitScore)
{
	if (inputList.size() != scoreList.size())
	{
		std::cout << "sublisting impossible, different size lists\n";
		return {};
	}

	std::vector<std::vector<T>> sublists;
	std::vector<T> currentSubList;
	int currentScore = 0;
	for (size_t i = 0; i < scoreList.size(); i++)
	{
		int score = scoreList[i];

		if (currentScore + score > splitScore && !currentSubList.empty())
		{
			sublists.emplace_back(std::move(currentSubList));
			currentSubList.clear();
			currentScore = 0;
		}

		currentSubList.emplace_back(inputList[i]);
		currentScore += score;
	}
...
void CJGeoCreator::garbageCollection()
{
	...
	if (!finishedLoD02_ && settingsCollection.make02() ||
		...
		!finishedLoDd2_ && settingsCollection.maked2() ||
		!finishedLoD40_ && settingsCollection.make40() ||
		!finishedLoD41_ && settingsCollection.make41() ||
		!finishedLoD42_ && settingsCollection.make42())
	{
...
std::vector<CJGeoCreator::BuildingSurfaceCollection> CJGeoCreator::sortRoofStructures(const std::vector<TopoDS_Face>& roofOutlines, const std::vector<RCollection>& rCollectionList)
{
	...
	for (const RCollection& currentSurface : rCollectionList)
	{
		...
		for (const gp_Pnt& currentPoint : currentFacePoints)
		{
			...
			{
				if (helperFunctions::pointOnShape(currentSurfaceCol.getRoofOutline(), projectedPoint, SettingsCollection::getInstance().linearTolerance())) //TODO: check if this can be changed to normal tolerance
				{
...
std::vector<TopoDS_Face> CJGeoCreator::section2Faces(const std::vector<T>& shapes, double cutlvl)
{
	...
	gp_Pnt p0 = gp_Pnt(lll.X() - 10, lll.Y() - 10, cutlvl);
...
TopoDS_Solid CJGeoCreator::extrudeFace(const TopoDS_Face& evalFace, bool downwards, double splittingFaceHeight)
{
	...
	{		
		...
		for (BRepTools_WireExplorer expl(currentWire); expl.More(); expl.Next())
		{
			...
			if (edge.Orientation() != TopAbs_FORWARD) { std::swap(p0, p1); }
			...
			if (p2.IsEqual(p3, precision)) { continue; }
...
void CJGeoCreator::makeFootprint(DataManager* h)
{
	...
	std::vector<TopoDS_Face> cleanedFootprintList;
	try
	{
	...
	for (const TopoDS_Face& currentFootprint : cleanedFootprintList) //TODO: multithread
	{
		...
		for (const TopoDS_Face& otherFootprint : cleanedFootprintList)
		{
			if (currentFootprint.IsEqual(otherFootprint)) { continue; }
			if (helperFunctions::surfaceIsIncapsulated(currentFootprint, otherFootprint))
			{
				isEncapsulated = true;
				break;
			}
...
void CJGeoCreator::getSplitFaces(std::vector<TopoDS_Face>& outFaceList, std::mutex& listMutex, const std::vector<TopoDS_Face>& inputFaceList, bgi::rtree<std::pair<BoostBox3D, TopoDS_Face>, bgi::rstar<25>> cuttingFaceIdx)
{
	...
	for (const TopoDS_Face& currentRoofSurface : inputFaceList)
	{
		...
		if (!currentNormal.IsParallel(horizontalNormal, precision))
		{
			std::optional<gp_Pnt> optionalbasePoint = helperFunctions::getPointOnFace(currentRoofSurface);

			if (optionalbasePoint == std::nullopt) { continue; }

			gp_Vec normalCrossProduct = currentNormal ^ horizontalNormal;
			gp_Ax1 rotationAxis(*optionalbasePoint, normalCrossProduct);
			Standard_Real rotationAngle = currentNormal.AngleWithRef(horizontalNormal, rotationAxis.Direction());
		...
		for (const auto& [cuttingBox, cuttingFace] : qResult)
		{
			...
			if (!otherNormal.IsParallel(currentNormal, angularTol)) { continue; }
			if (cuttingFace.IsEqual(currentRoofSurface)) { continue; }
		...
		for (TopExp_Explorer expl(divider.Shape(), TopAbs_FACE); expl.More(); expl.Next()) {
			...
			BRepBuilderAPI_Transform transformer3(subFace, transform);
			TopoDS_Face returnSubFace = TopoDS::Face(transformer3.Shape());

			listMutex.lock();
			outFaceList.emplace_back(returnSubFace);
			listMutex.unlock();
		}
	}
...
std::vector<TopoDS_Face> CJGeoCreator::getVisTopSurfaces(const std::vector<TopoDS_Face>& faceList, double lowestZ, const std::vector<TopoDS_Face>& bufferSurfaceList)
{
	...
	for (const TopoDS_Face& currentFace : faceList)
	{
		...
		if (!helperFunctions::hasVolume(bbox)) { continue; }
...
std::vector<TopoDS_Shape> CJGeoCreator::computePrisms(const std::vector<TopoDS_Face>& inputFaceList, double lowestZ, bool preFilter, const std::vector<TopoDS_Face>& bufferSurfaceList)
{
	...
	for (const TopoDS_Face& currentFace : splitTopSurfaceList)
	{
		...
		for (TopExp_Explorer expl(extrudedShape, TopAbs_FACE); expl.More(); expl.Next()) {
			...
			if (abs(currentNormal.Z()) > angularTolerance) 
			{ 
				horizontalFaceList.emplace_back(extrusionFace);
				continue; 
			}
	...
	for (const auto& [currentBox, currentFace] : SplitfaceIdx)
	{
		...
		for (const auto& [otherBox, otherFace] : qResult)
		{
			...
			if (!helperFunctions::isSame(currentFace, otherFace)) { continue; }
			isDub = true;
			break;
		}
		if (!isDub) { brepSewer.Add(currentFace); }
	}

	// add the horizontal faces (both roof and projects)
	for (const TopoDS_Face& currentFace : horizontalFaceList)
	{
...
std::vector<TopoDS_Face> CJGeoCreator::getSplitTopFaces(const std::vector<TopoDS_Face>& inputFaceList, double lowestZ, const std::vector<TopoDS_Face>& bufferSurfaceList)
{
	...
	bgi::rtree<std::pair<BoostBox3D, TopoDS_Face>, bgi::rstar<25>> inputFaceIdx; // pair bbox | extruded shape faces
	for (const TopoDS_Face& currentTopFace : inputFaceList)
	{
		...
		inputFaceIdx.insert(std::make_pair(topFaceBox, currentTopFace));

		if (!allFlat) { continue; }
		gp_Vec currentNormal = helperFunctions::computeFaceNormal(currentTopFace);
		if (currentNormal.IsParallel(horizontalVec, angularTolerance)) { continue; }
	...
	if (!allFlat)
	{
		...
		for (const TopoDS_Face& currentTopFace : trimmedFace)
		{
			if (currentTopFace.IsNull()) { continue; }
			BoostBox3D topFaceBox = helperFunctions::createBBox(currentTopFace, precision);
...
std::vector<CJT::GeoObject> CJGeoCreator::makeLoD40(DataManager* h, CJT::Kernel* kernel, int unitScale)
{
	std::cout << CommunicationStringEnum::getString(CommunicationStringID::infoComputingLoD40) << std::endl;
...
std::vector<CJT::GeoObject> CJGeoCreator::makeLoD41(DataManager* h, CJT::Kernel* kernel, int unitScale)
{
	SettingsCollection& settingsCollection = SettingsCollection::getInstance();
	...
	if (settingsCollection.createOBJ())
	{
		helperFunctions::writeToOBJ(collectionShape, settingsCollection.getOutputBasePath() + fileExtensionEnum::getString(fileExtensionID::OBJLoD41));
	}
...
std::vector<CJT::GeoObject> CJGeoCreator::makeLoD42(DataManager* h, CJT::Kernel* kernel, int unitScale)
{
	...
	std::vector< CJT::GeoObject> geoObjectList; // final output collection
	std::vector<TopoDS_Shape> collectionShape;
	brepIFcElemToGeoObject(h, kernel, geoObjectList, collectionShape);
	
	SettingsCollection& settingsCollection = SettingsCollection::getInstance();
	if (settingsCollection.createOBJ())
	{
		helperFunctions::writeToOBJ(collectionShape, settingsCollection.getOutputBasePath() + fileExtensionEnum::getString(fileExtensionID::OBJLoD42));
	}

	if (settingsCollection.createSTEP())
	{
		helperFunctions::writeToSTEP(collectionShape, settingsCollection.getOutputBasePath() + fileExtensionEnum::getString(fileExtensionID::STEPLoD42));
	}
...
std::vector< CJT::GeoObject>CJGeoCreator::makeLoD32(DataManager* h, CJT::Kernel* kernel, int unitScale)
{
	...
	for (size_t i = 0; i < finalOuterSurfacePairList.size(); i++)
	{
		...
		gp_Vec currentVec = helperFunctions::computeFaceNormal(currentFace);
		if (currentVec.Magnitude() < settingsCollection.linearTolerance())
		{
...
void CJGeoCreator::makeComplexLoDRooms(DataManager* h, CJT::Kernel* kernel, std::vector<std::shared_ptr<CJT::CityObject>>& roomCityObjects, int unitScale) {	
	...
	for (auto spaceIt = spaceList.begin(); spaceIt != spaceList.end(); ++spaceIt)
	{
		...
		for (TopExp_Explorer explorer(spaceShape, TopAbs_FACE); explorer.More(); explorer.Next())
		{
			...
			gp_Vec faceNormal = helperFunctions::computeFaceNormal(currentFace);
			if (faceNormal.Magnitude() < SettingsCollection::getInstance().linearTolerance()) { continue; }
...
std::vector<TopoDS_Face> CJGeoCreator::projectionSplitting(const std::vector<TopoDS_Face>& inputFaceList, const bgi::rtree<std::pair<BoostBox3D, TopoDS_Face>, bgi::rstar<25>>& faceIdx)
{
	// split the range over cores
	int coreUse = SettingsCollection::getInstance().threadcount() - 1;
	if (coreUse > inputFaceList.size()) { coreUse = inputFaceList.size(); }
	int splitListSize = static_cast<int>(floor(inputFaceList.size() / coreUse));

	std::vector<std::thread> threadList;
	std::mutex processMutex;

	std::vector<TopoDS_Face> trimmedFaceList;
	for (size_t i = 0; i < coreUse; i++)
	{
		auto startIdx = inputFaceList.begin() + i * splitListSize;
		auto endIdx = (i == coreUse - 1) ? inputFaceList.end() : startIdx + splitListSize;
		...
		threadList.emplace_back([&, sublist]() {projectionSplitting(sublist, faceIdx, processMutex, trimmedFaceList); });
	}
...
void CJGeoCreator::projectionSplitting(const std::vector<TopoDS_Face>& inputFaceList, const bgi::rtree<std::pair<BoostBox3D, TopoDS_Face>, bgi::rstar<25>>& faceIdx, std::mutex& processMutex, std::vector<TopoDS_Face>& splitFaceListOut)
{
	double precision = SettingsCollection::getInstance().linearTolerance();
	double angularTolerance = SettingsCollection::getInstance().angularTolerance();
	for (const TopoDS_Face& currentTopFace : inputFaceList)
	{
		...
		{
			processMutex.lock();
			splitFaceListOut.emplace_back(currentTopFace);
			processMutex.unlock();
			continue;
		}
		...
		if (currentNormal.IsParallel(gp_Vec(0, 0, 1), angularTolerance))
		{
			...
			for (TopExp_Explorer expl(trimmedShape, TopAbs_FACE); expl.More(); expl.Next()) {
				TopoDS_Face subFace = TopoDS::Face(expl.Current());
				//if (helperFunctions::computeArea(subFace) < precision) { continue; }
				subFace.Move(translation);
				if (subFace.IsNull()) { continue; }
				processMutex.lock();
				splitFaceListOut.emplace_back(subFace);
				processMutex.unlock();
			}
			continue;
		}
		...
		for (TopExp_Explorer expl(trimmedShape, TopAbs_FACE); expl.More(); expl.Next()) {
			...
			processMutex.lock();
			splitFaceListOut.emplace_back(flatSubFace);
			processMutex.unlock();
		}
	}
	return;
...
std::vector<TopoDS_Face> CJGeoCreator::intersectionSplitting(const std::vector<TopoDS_Face>& inputFaceList, const bgi::rtree<std::pair<BoostBox3D, TopoDS_Face>, bgi::rstar<25>>& faceIdx)
{
	...
	for (size_t i = 0; i < coreUse; i++)
	{
		...
		threadList.emplace_back([&, sublist]() {intersectionSplitting(sublist, faceIdx, processMutex, trimmedFaceList); });
	}
...
void CJGeoCreator::intersectionSplitting(const std::vector<TopoDS_Face>& inputFaceList, const bgi::rtree<std::pair<BoostBox3D, TopoDS_Face>, bgi::rstar<25>>& faceIdx, std::mutex& processMutex, std::vector<TopoDS_Face>& splitFaceListOut)
{
	double precision = SettingsCollection::getInstance().linearTolerance();
	double angularTolerance = SettingsCollection::getInstance().angularTolerance();
	...
	{
		if (currentTopFace.IsNull()) { continue; }
		BoostBox3D bBox = helperFunctions::createBBox(currentTopFace, precision);

		std::vector<std::pair<BoostBox3D, TopoDS_Face>> qResult;
		faceIdx.query(bgi::intersects(bBox), std::back_inserter(qResult));
		if (qResult.size() <= 1)
		{
			processMutex.lock();
			splitFaceListOut.emplace_back(currentTopFace);
		...
		{
			if (currentTopFace.IsSame(otherFace)) { continue; }
			if (otherFace.IsNull()) { continue; }
			trimFaces.Append(otherFace);
		}
...
void CJGeoCreator::valueToGeoObject(
	DataManager* h, 
	CJT::Kernel* kernel, 
...
void CJGeoCreator::createSemanticData(CJT::GeoObject* geoObject, const TopoDS_Shape& geometryShape, bool isExterior)
{
	...
	for (TopExp_Explorer faceExp(geometryShape, TopAbs_FACE); faceExp.More(); faceExp.Next()) {
		...
		gp_Vec faceNormal = helperFunctions::computeFaceNormal(face);
		if (abs(faceNormal.Z()) > settingsCollection.linearTolerance())
		{
			if (helperFunctions::getHighestZ(face) - lowestShapeZ < settingsCollection.linearTolerance()) // is floor
			{
...
{ //TODO: this can be smarter
	...
	BOPAlgo_Splitter divider;
	divider.SetFuzzyValue(SettingsCollection::getInstance().linearTolerance());
	divider.SetRunParallel(Standard_False);
...
void CJGeoCreator::init(DataManager* h, double vSize)
{
	// compute generic voxelfield data
	try
	{
